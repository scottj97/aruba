  ...
  module Platforms
    ...
    class UnixEnvironmentVariables
      ...
      class UpdateAction
        ...
        def call(env)
          if block
            UnixEnvironmentVariables.new(env.update(other_env, &block))
          else
            UnixEnvironmentVariables.new(env.update(other_env))
          end
        end
      end
      ...
      class RemoveAction
        ...
        def call(env)
          ...
          UnixEnvironmentVariables.new(env)
        end
      end
      ...
      def prepared_environment
        if RUBY_VERSION <= '1.9.3'
          # rubocop:disable Style/EachWithObject
          actions.inject(ENV.to_hash.merge(env)) { |a, e| a = e.call(a) }
          # rubocop:enable Style/EachWithObject
        else
